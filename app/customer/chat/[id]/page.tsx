            ...
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                <div className="w-10 h-10 rounded-full overflow-hidden mr-3">
                  <Image
                    src={pharmacy.avatar || "/placeholder.svg"}
                    alt={pharmacy.name}
                    width={40}
                    height={40}
                ...
                <div>
                  <h2 className="font-medium">{pharmacy.name}</h2>
                  <p className="text-xs opacity-80">{pharmacy.lastSeen ? `${pharmacy.lastSeen}` : "Offline"}</p>
                </div>
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  <div
                    className={`max-w-[70%] rounded-lg p-3 ${
                      message.sender === "customer" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-800"
                    }`}
                  >
                    <p className="whitespace-pre-line">{message.content}</p>
                    <div
                      className={`text-xs mt-1 ${message.sender === "customer" ? "text-blue-200" : "text-gray-500"}`}
                    >
                      {message.timestamp.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                      {message.sender === "customer" && <span className="ml-1">{message.seen ? "Seen" : ""}</span>}
                    </div>
                  </div>
